...
function generateMaps(
    ...
    if (regenerate && currentGameMap) {
        ...
        gameMap = Array.from({ length: mapSize }, () =>
            Array(mapSize).fill("-"),
        );
        gameMap[0][0] = "O";
    }
            ...
            if (participants.size === roles.length) {
                ...
                setTimeout(async () => {
                    ...
                    const handleVanguardMove = async () => {
                        ...
                        const vanguardId = Array.from(
                            participants.entries(),
                        ).find(([, role]) => role === "Vanguard")?.[0];
                        if (!vanguardId) {
                            gameState.isActive = false;
                            stopAllCollectors();
                            await heistChannel
                        ...
                        moveCollector.on("collect", async (collected) => {
                            ...
                            if (validMoves.includes(move)) {
                                ...
                                if (!result) {
                                    error(
                                        "Error: moveVanguard did not return a result.",
                                    );
                                    return;
                                }
